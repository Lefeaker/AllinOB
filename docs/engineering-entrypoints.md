...
- `npm run quality`
  ...
  - `audit:release-surface:report` 的 `Forbidden Machine-Local Content` hard gate 会扫描 dist 与传入 archive 的文本成员，拒绝 user-home / root machine-local families：macOS `/Users/<user>/...`、Linux `/home/<user>/...` / `/root/...` 与 Windows drive `C:\\Users\\...` / `C:/Users/...`。扫描视图会解析 JSON / source map 字符串并规范 ASCII code escape 与 `%HH` encoding、识别任意/空 authority 的 `file:` URI、`webpack:` / `vite:` local-source URI 与 YAML；普通 HTTP(S) / WSS URL 和相对引用不应误报。该口径不声称覆盖 `/workspace`、`D:/a`、`/private/var` 等所有绝对路径；JSON / Markdown 报告会保留 exact artifact path 与 pattern id
  - 同一 release-surface gate 的 `Forbidden Inline Source Maps` section 会拒绝 JavaScript / CSS / HTML release members 中的 `sourceMappingURL=data:` inline source map；Markdown / text documentation examples 不属于该 executable/style carrier gate
...
- `npm run build*` 与 `npm run package*`
  ...
  - `scripts/build.mjs` 支持 opt-in `--overlay-manifest <json>` owner overlay build；默认 public entrypoints / manifest / static assets 不变。Overlay manifest validation canonicalizes filesystem paths, constrains external roots, validates known entrypoint replacements, manifest patch files, static copy rules, recursive static-copy contents, critical overwrite allowlists, and the banned permission set `unlimitedStorage` / `message_serialization` / `tabCapture`。构建继续以 canonical real path 作为读取、watch 与 root 校验 authority，但传给 esbuild 的 public / declared overlay source identity 只包含稳定 root-relative logical path；relative overlay import 不得跨 root，`?inline` CSS 同样绑定 stable logical identity。Release acceptance 必须在不同 physical roots 比较 exact output member paths 与 bytes，不得通过移除 `[hash]`、关闭 splitting 或产物后改名掩盖差异。
  ...
  - `tools/report-release-surface.mjs` / `tools/audit-release-archive.mjs` continue to audit manifest references, dev/test harness leakage, pseudo-locale leakage, secret-like files/content, the documented user-home/root machine-local path families, and inline source maps for both default and overlay build surfaces.
